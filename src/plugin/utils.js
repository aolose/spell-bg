const markStr = '\x03\x04\x05\x06\x07\x0E\x0F\x10\x11\x12';
const baseStr =
  '\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c' +
  '\x1d\x1e\x1f\x7f\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b' +
...
export const s2n = (a) => {
  if (a < len) return baseStr[a];
  const v = a % len;
  return markStr[(a - v) / len] + baseStr[v];
};
export const n2s = (str) => {
  if (str.length === 1) return baseStr.indexOf(str);
  return len * [markStr.indexOf(str[0])] + baseStr.indexOf(str[1]);
};
